...
export async function getTikTokInsight(
  ...
  try {
    ...
    return {
      ...
      thumbnail:
        item.video?.cover || item.video?.originCover || item.cover || null,
    };
  } catch (error) {
...
export async function getTikTokUserVideos(
  ...
  try {
    ...
    const items =
      postsJson.data?.itemList || postsJson.itemList || postsJson.items || [];
    ...
    const videos: TikTokVideo[] = items.map((item: Record<string, unknown>) => {
      ...
      return {
        ...
        thumbnail:
          (video.cover as string) || (video.originCover as string) || null,
      };
    });
