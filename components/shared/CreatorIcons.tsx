...
export function CreatorIcons({
  ...
  const [selectedCreator, setSelectedCreator] = useState<CreatorInfo | null>(
    null,
  );
    ...
    return (
      <>
        <div
          ...
          onClick={
            clickable ? () => setSelectedCreator(defaultCreator) : undefined
          }
        >
...
export function SingleCreatorIcon({
  ...
  if (!creator) {
    return (
      <div
        className={`${config.icon} rounded-sm bg-emerald-600 flex items-center justify-center text-white ${config.text} font-medium shrink-0`}
      >
        BT
      </div>
    );
