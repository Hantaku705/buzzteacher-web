  ...
  try {
    ...
    if (plusBtnCount > 0) {
      ...
      const selectedText = await page
        .locator("text=「ど素人ホテル」の視点でアドバイスします")
        .count();
      ...
      console.log(
        `+ button has emerald color: ${btnClass?.includes("emerald") ? "Yes" : "No"}`,
      );
    }

    // スクリーンショットを保存
    await page.screenshot({
      path: "/Users/hantaku/Downloads/BuzzTeacher/web/test-plus-button.png",
      fullPage: true,
    });
    ...
    await page.screenshot({
      path: "/Users/hantaku/Downloads/BuzzTeacher/web/test-plus-error.png",
      fullPage: true,
    });
  } finally {
    await browser.close();
