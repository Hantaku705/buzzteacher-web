...
test.describe("BuzzTeacher Chat", () => {
  ...
  test("should send a message and receive response", async ({ page }) => {
    ...
    await expect(assistantMessage.locator(".flex-1")).not.toBeEmpty({
      timeout: 60000,
    });
  });
  ...
  test("should handle TikTok URL input", async ({ page }) => {
    ...
    const tiktokUrl =
      "https://www.tiktok.com/@4610_hotel/video/7401449837133161761";
  ...
  test("should handle TikTok profile URL input", async ({ page }) => {
    ...
    await expect(page.getByText(tiktokProfileUrl)).toBeVisible({
      timeout: 10000,
    });
  ...
  test("should support multiple messages in conversation", async ({ page }) => {
    ...
    await expect(page.locator('[class*="bg-[#444654]"]').first()).toBeVisible({
      timeout: 60000,
    });
